...
then
    ...
    # 4c. mcRegistry — stock layout /vendor/app/mcRegistry (drbin + tabin).
    #    Source: prebuilt/recovery_vendor_hal/app_mcRegistry (pulled from the
    #    stock system) when present, else the legacy curated mcRegistry set.
    #    KeyMint TA: keep ONLY the latest patch-level TA (9598; 5228 is 12 MB
    #    and superseded).  All .drbin drivers are always kept — mcDriverDaemon
    #    preloads them via -r at startup.
    #    UUID prefix mapping:
    #      050600 = KeyMint, 060800 = Gatekeeper, 070f = Weaver,
    #      070600 = Keymaster (legacy), 077700 = Secure Storage,
    #      090400 = Secure Element (SN100 eSE)
    mc_src="$vendor_hal_dir/app_mcRegistry"
    [ -d "$mc_src" ] || mc_src="$vendor_hal_dir/mcRegistry"
    if [ -d "$mc_src" ]; then
        install -d -m 0755 "$root/vendor/app/mcRegistry"
        for ta in "$mc_src"/*
        do
            [ -e "$ta" ] || continue
            name="$(basename "$ta")"
            case "$name" in
                # Kinibi driver binaries — always preloaded by the daemon
                *.drbin) ;;
            ...
            cp -a -- "$ta" "$root/vendor/app/mcRegistry/"
        done

        # Generate the mobicore daemon launcher with -r for every staged
        # driver binary (the stock trustonic.rc passes them explicitly).
        # Wait up to 60s for /dev/mobicore: load-modules.sh insmods
        # mcDrvModule-ffa.ko during on-fs, which may finish around the same
        # time as this service starts.
        launcher="$root/system/bin/start-mobicore.sh"
        dr_args=""
        for dr in "$root/vendor/app/mcRegistry"/*.drbin
        do
            [ -e "$dr" ] || continue
            dr_args="$dr_args -r /vendor/app/mcRegistry/$(basename "$dr")"
        done
        {
            echo '#!/system/bin/sh'
            echo '# Generated by recovery/prepare-ramdisk.sh — do not edit.'
            echo 'i=0'
            echo 'while [ ! -e /dev/mobicore ] && [ $i -lt 60 ]; do'
            echo '    sleep 1'
            echo '    i=$((i+1))'
            echo 'done'
            echo 'if [ ! -e /dev/mobicore ]; then'
            echo '    echo "start-mobicore: /dev/mobicore not present, aborting" >&2'
            echo '    exit 1'
            echo 'fi'
            echo "exec /vendor/bin/mcDriverDaemon --P1 /mnt/vendor/persist/mcRegistry$dr_args"
        } > "$launcher"
        chmod 0755 "$launcher"
        echo "pd2415 recovery ramdisk preparation: generated start-mobicore.sh with $(ls "$root/vendor/app/mcRegistry"/*.drbin 2>/dev/null | wc -l) drbin(s)"
    fi
fi
...
# 5d. Make the helper scripts executable (git on Windows does not keep the
...
install -m 0755 "$device_root/recovery/root/system/bin/mount-persist.sh" \
    "$root/system/bin/mount-persist.sh"
